...
impl Column {
	...
	pub fn open(col: ColId, options: &Options, salt: Option<Salt>) -> Result<Column> {
		...
		let tables = Tables {
			index,
			value: [
				Self::open_table(path, col, 0, Some(options.sizes[0]), &options)?,
				Self::open_table(path, col, 1, Some(options.sizes[1]), &options)?,
				Self::open_table(path, col, 2, Some(options.sizes[2]), &options)?,
				Self::open_table(path, col, 3, Some(options.sizes[3]), &options)?,
				Self::open_table(path, col, 4, Some(options.sizes[4]), &options)?,
				Self::open_table(path, col, 5, Some(options.sizes[5]), &options)?,
				Self::open_table(path, col, 6, Some(options.sizes[6]), &options)?,
				Self::open_table(path, col, 7, Some(options.sizes[7]), &options)?,
				Self::open_table(path, col, 8, Some(options.sizes[8]), &options)?,
				Self::open_table(path, col, 9, Some(options.sizes[9]), &options)?,
				Self::open_table(path, col, 10, Some(options.sizes[10]), &options)?,
				Self::open_table(path, col, 11, Some(options.sizes[11]), &options)?,
				Self::open_table(path, col, 12, Some(options.sizes[12]), &options)?,
				Self::open_table(path, col, 13, Some(options.sizes[13]), &options)?,
				Self::open_table(path, col, 14, Some(options.sizes[14]), &options)?,
